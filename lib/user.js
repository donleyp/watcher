...
class UserHandler extends Handler {
    ...
    async post(data) {
        ...
        let { firstName, lastName, phone, email, password, tosAccepted } = data.payload;
        ...
        email = email && validateEmail(email) ? email.trim() : false;
     ...
     */
    async put(data) {
        let { firstName, lastName, phone, email, password } = data.payload;
        firstName = firstName && firstName.trim().length > 0 ? firstName.trim() : false;
        lastName = lastName && lastName.trim().length > 0 ? lastName.trim() : false;
        phone = phone && phone.trim().length == 10 ? phone.trim() : false;
        email = email && validateEmail(email) ? email.trim() : false;
    ...
    async delete(data) {
        ...
        if (phone) {
            ...
            if (userData.checks && userData.checks.length > 0) {
                // delete each of the checks.
                const deleteChecks = userData.checks.map(async checkId => {
                    // return the promise to delete the check, catch and log errors.
                    return checks.delete(checkId).catch(err => console.log);
                });
