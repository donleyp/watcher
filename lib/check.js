...
class CheckHandler extends Handler {
    ...
    extractFields(payload) {
        ...
        timeoutSeconds = typeof(timeoutSeconds) == 'number' && timeoutSeconds >= 1 && timeoutSeconds <= 5 ? timeoutSeconds : false;
    ...
    async post(data) {
        ...
        if (protocol && url && method && successCodes && timeoutSeconds) {
            ...
            if (userChecks.length < config.maxChecks) {
                ...
                await checks.create(id, checkObject);
    ...
    async get(data) {
        ...
        if (id) {
            try {
                const checkData = await checks.read(id);
            ...
            const filteredCheckIdList = checkIdList.filter(async (id, idx, array) => {
                const checkData = await checks.read(id);
                return data.tokenData.phone === checkData.userPhone;
            });
            return this.clientResponse(filteredCheckIdList);
        }
    }
    ...
    async put(data) {
        ...
        if (id && (protocol || url || method || successCodes || timeoutSeconds)) {
            ...
            if(data.tokenData.phone === checkData.userPhone) {
                ...
                await checks.update(id, checkData);
                return this.clientResponse(checkData);
            } else {
                return this.clientError(403);
            }
