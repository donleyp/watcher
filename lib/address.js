...
class AddressHandler extends Handler {
    ...
    extractFields(payload) {
        ...
        street1 = street1 && validateNonEmpty(street1) ? street1.trim() : false;
        street2 = street2 && validateNonEmpty(street2) ? street2.trim() : false;
        city = city && validateNonEmpty(city) ? city.trim() : false;
        state = state && validateNonEmpty(state) ? state.trim() : false;
        zip = zip && validateDigits(zip) ? zip.trim() : false;
