...
class Server {
    constructor() {
        ...
        this.router = {
          ...
          users: userHandler.getHandler(),
          tokens: tokenHandler.getHandler(),
          checks: checkHandler.getHandler(),
          addresses: addressHandler.getHandler(),
        };
    }
    ...
    async tokenHandler(data) {
        if(data.headers.token) {
            try {
                const tokenData = await tokens.read(data.headers.token);
                if (tokenData.expires >= Date.now()) {
                    data.tokenData = tokenData;
                    try {
                        const userData = await users.read(tokenData.phone);
                        // remove the hashed password so it doesn't get logged anywhere.
                        delete userData.hashedPassword;
                        data.userData = userData;
                    } catch (err) {
                        console.log(`Valid token, but no user: ${err.code}`);
                    }
                } else {
